import { NutrientStats } from "../entries/entry.ts"
...
export function calc_day_goals(goalinfo: { multiplier: number, goalLbs: number, proteinGPerLBS: number, fatGPerLBS: number }): NutrientStats {
    ...
    let day_nutrients: NutrientStats = new NutrientStats(
        goalinfo.goalLbs * goalinfo.multiplier,
        Math.round(goalinfo.proteinGPerLBS * goalinfo.goalLbs),
        Math.round(goalinfo.fatGPerLBS * goalinfo.goalLbs),
        0
    )
...
export function calc_pct_goals(totals: NutrientStats, pct: number): NutrientStats {
    let nutrients: NutrientStats = new NutrientStats(
        Math.round(totals.cal * pct),
        Math.round(totals.protein * pct),
        Math.round(totals.fat * pct),
        Math.round(totals.carbs * pct)
    )
    return nutrients
}
...
export function parse_goals(goalinfo: { "goalLbs": number, "multiplier": number, "acceptablePercent": number, "goalsPerMeal": string, "proteinGPerLBS": number, "fatGPerLBS": number }): GoalInfo {
    ...
    all_calculated["totals"] = { "Total": day_goals }
    all_calculated["marginOfError"] = { "Total": err_daygoals }
